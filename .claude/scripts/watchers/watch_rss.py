...
Ported from NousResearch/hermes-agent optional-skills/devops/watchers (MIT).

Usage (OS scheduler or manual):

    cd .claude/scripts && uv run python watchers/watch_rss.py \\
      --name hn --url https://news.ycombinator.com/rss

First run records a baseline (emits nothing).  Subsequent runs emit only
items whose <guid> / <id> isn't in the watermark.  Empty stdout means
"nothing new" — wrap in a scheduled job that skips delivery on empty
output (the [SILENT] contract).
"""
...
def main() -> int:
    ...
    try:
        req = urllib.request.Request(args.url, headers={"User-Agent": "TheHomie-Watcher/1.0"})
    ...
    # Empty stdout on no-new — scheduled wrapper treats that as silent.
    return 0
