"""Shared watermark helper used by the watcher scripts.

Ported from NousResearch/hermes-agent optional-skills/devops/watchers
(MIT). Behavior kept verbatim; only state-dir resolution and branding
are adapted to The Homie.
...
def _state_dir() -> Path:
    ...
    Default resolves through config.STATE_DIR (the persona-aware path
    resolver) at call time — never a raw install-dir join.
    """
    ...
    import config  # noqa: PLC0415 — call-time resolution (Rule 1)

    return Path(config.STATE_DIR) / "watchers"
...
def format_items_as_markdown(
    ...
    *,
    ...
    """Render a list of items as Markdown for scheduled delivery.

    One heading per item + its URL + optional snippet of body.  Output is
    empty string when items is empty — the scheduled-job wrapper treats
    empty stdout as silent and skips delivery ([SILENT] contract).
    """
    if not items:
        return ""
    lines: List[str] = []
