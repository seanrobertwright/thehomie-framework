...
**The event-loop wedge (2026-07-13) + the browser-write rule.** A social approve tap ran the whole 60–120s browser drive synchronously inside the async handler; a hung agent-browser child blocked `communicate()` forever and froze Telegram, Discord, `/health`, and the in-process liveness supervisor at once (they all share the loop — the smoke detector was in the burning room; only the external watchdog can catch this class). The rule now enforced framework-wide: **the bot never drives a browser on its event loop.** Queue-backed posts (`/social post`, approve taps) CAS-claim the row and spawn the detached **Browser Homie runner** (`../scripts/social/browser_homie_runner.py` — receipts return via Telegram, stale claims swept by the cadence tick); per-action writes (`/linkedin_post`, `/reddit comment|post`) and browser reads ride `asyncio.to_thread` with a bounded reply; all writes serialize on the cross-process `shared.browser_write_lock`. Diagnose any future wedge with `uvx py-spy dump --pid <bot pid>`. Full design: `docs/manual/features/social-post-pipeline.md` → Browser Homie Runner.

**One launcher:** `run_chat.sh` (Git Bash on Windows) is the ONLY launcher. `run_chat.bat` was retired 2026-07 (archived to `.claude/_archive/lifecycle-2026-07/` along with `service.py`/`run_service.bat`) — it hardcoded `--telegram` and silently resurrected a Telegram-only bot with no Discord and no relay. If Git Bash is missing the watchdog now FAILS LOUD (toast + launcher-log receipt) instead of falling back. Operator on/off intent lives in the desired-state switch (`thehomie on|off`) — see `docs/manual/features/bot-lifecycle.md`.

**Watchdog-cadence trap:** if the bot "keeps turning off" for hours at a time, check `schtasks /query /tn "SecondBrain-BotWatchdog" /v | findstr "Repeat"` FIRST — on 2026-07-13 something rewrote the task's repetition from 5 minutes to hourly (`PT1H`), turning every wedge into a ~2h outage. The watchdog is a one-shot poll (~2s, zero resident RAM); its cadence is the entire recovery SLA.
