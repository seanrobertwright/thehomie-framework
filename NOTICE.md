# Notice

The Homie is the original public Homie framework export at:

https://github.com/SmokeAlot420/thehomie-framework

Maintained by SmokeDev and The Homie contributors.

First public preview release: `v0.1.0-alpha.1`, June 5, 2026.

## Lineage

The Homie evolved from Cole and the Dynamous Community's Claude Code Second
Brain workshop, then grew into an identity-first cognitive agent OS with its
own memory, orchestration, multi-channel ingress, Operating Room, and
desktop/operator surfaces.

OpenClaw is credited for proving broad agent/channel access. Hermes Agent is
credited for pushing self-improving agent loops and strong provider/tooling
ergonomics.

The Homie is an independent project. These lineage credits do not imply
affiliation, sponsorship, endorsement, or ownership by Dynamous Community,
OpenClaw, Hermes Agent, Nous Research, or their maintainers.

## License

The Homie is released under the MIT License. See `LICENSE`.
