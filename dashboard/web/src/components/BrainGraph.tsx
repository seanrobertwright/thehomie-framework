...
function lobeFor(entry: HiveEntry): Lobe {
  return LOBE_BY_ID[AGENT_LOBE[entry.agent_id] || fallbackLobeId(entry.agent_id)];
}
...
function generateLobePools(cerebrum: SVGPathElement): LobePools {
  ...
  for (const lobe of LOBES) {
    ...
    while (pts.length < target && tries < target * 60) {
      ...
      const x = x0 + r() * w;
      const y = y0 + r() * h;
      if (!pointInCerebrum(cerebrum, x, y)) continue;
    ...
    if (pts.length === 0) {
      ...
      for (let row = 0; row < rows; row++) {
        for (let col = 0; col < cols; col++) {
          const x = x0 + ((col + 0.5) / cols) * w;
          if (Math.abs(x - 450) < 10) continue;
          const y = y0 + ((row + 0.5) / rows) * h;
          pts.push({ x, y });
        }
      }
    }
...
function pointInCerebrum(cerebrum: SVGPathElement, x: number, y: number): boolean {
  const svgPoint = cerebrum.ownerSVGElement?.createSVGPoint?.();
  if (svgPoint) {
    svgPoint.x = x;
    svgPoint.y = y;
    return cerebrum.isPointInFill(svgPoint);
  }
  return (cerebrum as any).isPointInFill({ x, y });
}
